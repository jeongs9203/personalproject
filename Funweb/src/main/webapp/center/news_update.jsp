	...
	<div id="wrap">
		...
		<article>
			...
			<form action="news_updatePro.jsp" method="post">
				...
				<input type="hidden" name="pass" value="<%=newsBean.getPass()%>">
				<table id="notice">
					<tr>
						<td>글쓴이</td>
						<td><input type="text" name="name" required="required" value="<%=newsBean.getName() %>"></td>
					</tr>
					<tr>
						<td>비밀번호</td>
					...
					<tr>
						<td>제목</td>
						<td><input type="text" name="subject" required="required" value="<%=newsBean.getSubject() %>"></td>
					</tr>
					<tr>
						<td>내용</td>
						<td><textarea rows="10" cols="20" name="content" required="required"><%=newsBean.getContent() %></textarea></td>
					</tr>
				</table>
